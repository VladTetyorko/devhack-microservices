...
/**
 * Test class for VacancyResponseServiceImpl.
 * Tests all vacancy response management functionality including search, filtering, and lifecycle management.
 */
...
class VacancyResponseServiceImplTest extends BaseServiceTest {
    ...
    void testMarkOutdatedResponsesRejectedStageNotFound() {
        ...
        verify(interviewStageService).findRejectedStage();
        verify(repository, never()).bulkMarkOutdated(any(), any(), any(), any(), any());
    }

    @Test
    @DisplayName("Should get entity user")
    @Description("Test that getEntityUser returns the user from vacancy response")
    @Severity(SeverityLevel.NORMAL)
    void testGetEntityUser() {
        // Given
        User user = createTestUser();
        VacancyResponse response = createTestVacancyResponse("TestCompany", "TestPosition");
        response.setUser(user);

        // When
        User result = vacancyResponseService.getEntityUser(response);
